...
export const Card = (() => {
    ...
    const proUrl = ')]'
    return (
        <div className='flex justify-between flex-1 h-fit flex-wrap gap-2 w-full px-[66px] py-5 border-t !border-t-[#ebebeb]'>
            { cardsIcons && cardsIcons.map((card, key) => (
                <>
                <div key={key} className='cursor-pointer hover:shadow-2xl transition rounded-xl p-[8px]'>
                    <div className={`${preUrl+card.urlCard+proUrl} relative w-[272px] h-[256px] bg-cover bg-center rounded-xl`}>
                        <div className='absolute top-3 right-3 bg-[#cfd3d9] text-[#222222] rounded-full'>
                            <FontAwesomeIcon icon={faArrowUpFromBracket} size='sm' className='p-2 px-[12px]'/>
                        </div>
                    </div>
